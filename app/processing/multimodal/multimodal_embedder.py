...
class MultimodalEmbedder:
    ...
    def _init_text_embedder(self):
        ...
        try:
            ...
            if settings.openai_api_key:
                ...
                self.text_client = AsyncOpenAI(api_key=api_key)
    ...
    async def embed_text_chunks(self, chunks: list[TextChunk]) -> list[EmbeddedChunk]:
    ...
    async def embed_visual_chunks(self, chunks: list[VisualChunk]) -> list[EmbeddedChunk]:
    ...
    async def embed_multimodal_chunks(self, chunks: list[MultimodalChunk]) -> list[EmbeddedChunk]:
        ...
        for chunk in chunks:
            try:
                ...
                if text_embedding and visual_embeddings:
                    fused_embedding = self.fuse_embeddings(text_embedding, visual_embeddings)
                elif text_embedding:
    ...
    def fuse_embeddings(self, text_emb: list[float], visual_embs: list[np.ndarray]) -> list[float]:
