...
class FileStorageService:
    ...
    def get_storage_path(self, file_id: str, original_filename: str) -> Path:
        ...
        storage_path = (self.upload_dir / filename).resolve()

        # Verify the path is inside upload_dir (prevent path traversal)
        try:
            storage_path.relative_to(self.upload_dir.resolve())
        except ValueError as exc:
            raise FileValidationError(
                "Invalid file path: resolved path is outside upload directory"
            ) from exc
