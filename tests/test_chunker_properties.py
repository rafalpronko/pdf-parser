...
def parsed_document_strategy(draw, min_text_blocks=1, max_text_blocks=10, include_images=True):
    ...
    for i in range(num_text_blocks):
        ...
        text_blocks.append(TextBlock(content=content, page=page, bbox=bbox, font_size=font_size))
    ...
    if include_images:
        ...
        for _ in range(num_images):
            ...
            images.append(
                ImageBlock(image_data=b"fake_image_data", page=page, bbox=bbox, format="png")
            )

    return ParsedDocument(
        text_blocks=text_blocks, images=images, tables=[], num_pages=num_pages, metadata={}
    )
...
class TestChunkingPreservesTotalContent:
    ...
    @given(
        parsed_doc=parsed_document_strategy(
            min_text_blocks=1, max_text_blocks=10, include_images=False
        ),
    ...
    def test_property_9_chunking_preserves_total_content(
        self, parsed_doc, chunk_size, chunk_overlap
    ):
    ...
    def test_property_9_chunk_count_is_reasonable(self, text_length, chunk_size, chunk_overlap):
        ...
        text_blocks = [TextBlock(content=text, page=0, bbox=(0.0, 0.0, 100.0, 100.0))]

        parsed_doc = ParsedDocument(
            text_blocks=text_blocks, images=[], tables=[], num_pages=1, metadata={}
        )
...
class TestChunkBoundaryIntegrity:
    ...
    @given(
        parsed_doc=parsed_document_strategy(
            min_text_blocks=1, max_text_blocks=5, include_images=False
        ),
...
class TestImageTextAssociation:
    ...
    @given(
        parsed_doc=parsed_document_strategy(
            min_text_blocks=1, max_text_blocks=5, include_images=True
        ),
        chunk_size=st.integers(min_value=100, max_value=500),
        chunk_overlap=st.integers(min_value=10, max_value=50),
    )
    ...
    def test_property_11_images_associated_with_nearby_chunks(
        self, num_pages, images_per_page, chunk_size
    ):
        """Feature: pdf-rag-system, Property 11: Image-text association.
        ...
        for page in range(num_pages):
            ...
            text_blocks.append(TextBlock(content=text, page=page, bbox=(0.0, 0.0, 100.0, 50.0)))
...
class TestStructureAwareChunking:
    ...
    def test_property_12_structure_aware_chunking_alignment(
        self, num_sections, chunk_size, chunk_overlap
    ):
        ...
        parsed_doc = ParsedDocument(
            text_blocks=text_blocks, images=[], tables=[], num_pages=3, metadata={}
        )
    ...
    @given(chunk_size=st.integers(min_value=50, max_value=200))
    @settings(
        max_examples=100, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
    def test_property_12_structure_aware_preserves_headings(self, chunk_size):
        ...
        text_blocks = [
            TextBlock(content="INTRODUCTION", page=0, bbox=(0.0, 0.0, 100.0, 20.0), font_size=16.0),
            TextBlock(
            ...
            TextBlock(content="METHODS", page=0, bbox=(0.0, 60.0, 100.0, 80.0), font_size=16.0),
            TextBlock(
