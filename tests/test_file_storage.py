...
class TestFileValidationProperties:
    ...
    @given(
        filename=st.text(min_size=1, max_size=100).filter(lambda x: not x.lower().endswith(".pdf")),
        file_size=st.integers(min_value=1, max_value=1000000000).filter(
    ...
    async def test_property_2_save_file_rejects_invalid_inputs(
        ...
        with pytest.raises(FileValidationError) as exc_info:
            await file_storage.save_file(
                file_content=file_content, filename=filename, content_type="application/pdf"
            )
        ...
        assert "Invalid file extension" in error_msg or "exceeds maximum allowed size" in error_msg

    @given(
        case_variant=st.sampled_from(
            ["document.pdf", "document.PDF", "document.Pdf", "document.pDf", "document.pdF"]
        )
    )
    @settings(
        max_examples=100, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
    def test_property_2_case_insensitive_extension_validation(self, file_storage, case_variant):
        """Feature: pdf-rag-system, Property 2: File validation rejects invalid inputs.
    ...
    @given(file_content=st.binary(min_size=1, max_size=50 * 1024 * 1024))
    @settings(
        max_examples=100, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
    @pytest.mark.asyncio
    async def test_property_2_valid_pdf_saves_successfully(self, file_storage, file_content):
        """Feature: pdf-rag-system, Property 2: File validation rejects invalid inputs.
