...
async def test_generate_with_system_message(mock_openai_client):
    ...
    # Test
    client = OpenAIClient(api_key="test-fake-api-key-for-unit-tests")
    result = await client.generate("test prompt", system_message="You are a helpful assistant")
...
async def test_generate_with_context(mock_openai_client):
    ...
    # Test
    client = OpenAIClient(api_key="test-fake-api-key-for-unit-tests")
    result = await client.generate_with_context(question="What is X?", context="X is a thing.")
...
async def test_retry_with_timeout_error(mock_openai_client):
    ...
    mock_instance = AsyncMock()
    mock_instance.embeddings.create = AsyncMock(side_effect=APITimeoutError("Request timeout"))
    mock_openai_client.return_value = mock_instance
...
async def test_retry_with_rate_limit_error(mock_openai_client):
    ...
    rate_limit_error = RateLimitError("Rate limit exceeded", response=mock_http_response, body=None)
...
class TestEmbeddingDimensionalityConsistency:
    ...
    @pytest.mark.asyncio
    async def test_property_13_single_embedding_dimensionality(self, text, mock_openai_client):
        """Feature: pdf-rag-system, Property 13: Embedding dimensionality consistency.
...
class TestRetryWithExponentialBackoff:
    ...
    @pytest.mark.asyncio
    async def test_property_16_retry_exhaustion(self, max_retries, mock_openai_client):
        """Feature: pdf-rag-system, Property 16: Retry with exponential backoff.
...
class TestTimeoutHandling:
    ...
    @given(timeout=st.floats(min_value=1.0, max_value=60.0))
    @settings(
        max_examples=100, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
    @pytest.mark.asyncio
    async def test_property_28_timeout_handling(self, timeout, mock_openai_client):
        ...
        # Test
        client = OpenAIClient(
            api_key="test-fake-api-key-for-unit-tests", timeout=timeout, max_retries=2
        )
    ...
    @given(operation=st.sampled_from(["embed_text", "embed_batch", "generate"]))
    @settings(
        max_examples=100, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
    @pytest.mark.asyncio
    async def test_property_28_timeout_handling_all_operations(self, operation, mock_openai_client):
        """Feature: pdf-rag-system, Property 28: Timeout handling.
    ...
    @pytest.mark.asyncio
    async def test_property_28_timeout_retry_behavior(self, max_retries, mock_openai_client):
        """Feature: pdf-rag-system, Property 28: Timeout handling.
    ...
    @given(num_timeouts=st.integers(min_value=1, max_value=2))
    @settings(
        max_examples=100, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
    @pytest.mark.asyncio
    async def test_property_28_timeout_recovery(self, num_timeouts, mock_openai_client):
        """Feature: pdf-rag-system, Property 28: Timeout handling.
