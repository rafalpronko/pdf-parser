...
@asynccontextmanager
async def _mock_lifespan(app):  # noqa: ARG001
    """Mock lifespan that sets up mock services instead of real ones."""
    import app.main as main_module

    mock_doc_service = MagicMock()
    mock_doc_service.close = AsyncMock()
    mock_query_service = MagicMock()
    mock_query_service.close = AsyncMock()

    main_module.document_service = mock_doc_service
    main_module.query_service = mock_query_service

    yield

    main_module.document_service = None
    main_module.query_service = None


@pytest.fixture(autouse=True)
def _patch_lifespan():
    """Patch the app lifespan to avoid ChromaDB initialization conflicts."""
...
class TestAPIErrorStatusCodes:
    ...
    def test_property_27_nonexistent_document_returns_404(self, doc_id):
        ...
        """
        with TestClient(app) as client, patch("app.main.document_service") as mock_service:
    ...
    def test_property_27_service_unavailable_returns_503(self):
        ...
        """
        with TestClient(app) as client, patch("app.main.document_service", None):
    ...
    def test_property_27_timeout_returns_503(self):
        ...
        """
        with TestClient(app) as client, patch("app.main.query_service") as mock_service:
            # Mock service to raise timeout error with "timeout" in message
            mock_service.query = AsyncMock(side_effect=Exception("Request timeout occurred"))
    ...
    def test_property_27_invalid_query_params_return_422(self, top_k):
        ...
        with TestClient(app) as client:
            response = client.post(
                "/api/v1/query", json={"question": "What is this about?", "top_k": top_k}
            )
