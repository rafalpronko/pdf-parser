...
def chunk_strategy(doc_id: str = None, chunk_id_prefix: str = "chunk"):
    ...
    return st.builds(
        ...
        doc_id=st.just(doc_id) if doc_id else st.text(min_size=1, max_size=50, alphabet=st.characters(min_codepoint=32, max_codepoint=126)),
        ...
        metadata=st.dictionaries(
            keys=st.text(min_size=1, max_size=20, alphabet=st.characters(min_codepoint=97, max_codepoint=122)),  # lowercase letters only
            values=st.one_of(
...
class TestEmbeddingStorageRoundTrip:
    ...
    async def test_property_14_embedding_storage_round_trip(
        self, vector_store, num_chunks
    ):
        ...
        for embedded_chunk in embedded_chunks:
            ...
            for key, value in chunk.metadata.items():
                assert key in retrieved.metadata, \
                    f"metadata key '{key}' missing after round-trip"
    ...
    async def test_property_14_round_trip_with_varying_chunk_counts(
        self, vector_store, num_chunks
    ):
        """Feature: pdf-rag-system, Property 14: Embedding storage round-trip.
        ...
        assert count == num_chunks, \
            f"Expected {num_chunks} chunks, found {count}"
...
class TestReprocessingIdempotence:
    ...
    async def test_property_15_reprocessing_with_different_chunk_counts(
        ...
        for i in range(num_chunks_first):
            ...
            embedding = [float(i) * 0.1] * 384
            first_chunks.append(EmbeddedChunk(chunk=chunk, embedding=embedding))
        ...
        for i in range(num_chunks_second):
            ...
            second_chunks.append(EmbeddedChunk(chunk=chunk, embedding=embedding))
...
class TestTransactionalConsistency:
    ...
    async def test_property_29_transactional_consistency_on_success(
        self, vector_store, num_chunks
    ):
        ...
        for i in range(num_chunks):
            retrieved = await vector_store.get_chunk(f"chunk_{i}")
            assert retrieved is not None, \
                f"Chunk {i} not found (inconsistent storage)"
    ...
    async def test_property_29_no_orphaned_chunks_after_deletion(
        self, vector_store, num_chunks
    ):
        """Feature: pdf-rag-system, Property 29: Transactional consistency.
        ...
        assert count_after == 0, \
            f"Found {count_after} orphaned chunks after deletion (expected 0)"
    ...
    @pytest.mark.asyncio
    async def test_property_29_empty_embeddings_list_fails_cleanly(
        self, vector_store
    ):
        """Feature: pdf-rag-system, Property 29: Transactional consistency.
        ...
        count = await vector_store.count_chunks(doc_id="test_doc")
        assert count == 0, \
            f"Found {count} chunks after failed operation (partial data detected)"
