...
def query_request_strategy(draw):
    ...
    question = draw(st.text(min_size=1, max_size=200, alphabet=st.characters(min_codepoint=32, max_codepoint=126)))
...
def search_result_strategy(draw, doc_id=None, min_score=0.0, max_score=1.0):
    ...
    if doc_id is None:
        doc_id = draw(st.text(min_size=1, max_size=50, alphabet=st.characters(min_codepoint=97, max_codepoint=122)))
    
    chunk_id = draw(st.text(min_size=1, max_size=50, alphabet=st.characters(min_codepoint=97, max_codepoint=122)))
    ...
    relevance_score = draw(st.floats(min_value=min_score, max_value=max_score, allow_nan=False, allow_infinity=False))
...
class TestPromptContainsQueryAndContext:
    ...
    @pytest.mark.asyncio
    @given(
        question=st.text(min_size=1, max_size=200, alphabet=st.characters(min_codepoint=32, max_codepoint=126)),
    ...
    async def test_property_18_prompt_contains_query_and_context(
        self, query_service, mock_openai_client, mock_vector_store, mock_document_service, question, num_chunks
    ):
        ...
        mock_document_service.get_document = AsyncMock(side_effect=lambda doc_id: create_doc_info(doc_id))
    ...
    @pytest.mark.asyncio
    @given(
        question=st.text(min_size=1, max_size=100, alphabet=st.characters(min_codepoint=32, max_codepoint=126)),
    )
    @settings(
        max_examples=100,
        deadline=None,
    ...
    async def test_property_18_context_includes_metadata(
        ...
        assert str(page_num) in captured_context, \
            f"Page number {page_num} not found in context"
...
class TestResponseIncludesSourceCitations:
    ...
    async def test_property_19_response_includes_source_citations(
        self, query_service, mock_openai_client, mock_vector_store, mock_document_service,
        question, num_sources
    ):
        ...
        for i, source in enumerate(response.sources):
            ...
            assert source.doc_id == f"doc_{i}", \
                f"Source {i}: doc_id mismatch"
            
            # filename should be present
            assert source.filename == f"doc_{i}.pdf", \
                f"Source {i}: filename mismatch"
            
            # page should be present and non-negative
            assert source.page >= 0, \
                f"Source {i}: page number {source.page} is negative"
...
class TestMultiSourceSynthesis:
    ...
    async def test_property_20_multi_source_synthesis(
        self, query_service, mock_openai_client, mock_vector_store, mock_document_service,
        question, num_docs, chunks_per_doc
    ):
    ...
    async def test_property_20_context_from_multiple_sources(
        self, query_service, mock_openai_client, mock_vector_store, mock_document_service, question, num_docs
    ):
        """Feature: pdf-rag-system, Property 20: Multi-source synthesis.
