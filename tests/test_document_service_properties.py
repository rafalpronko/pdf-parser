...
def document_metadata_strategy(draw):
    ...
    return DocumentMetadata(
        filename=filename, content_type="application/pdf", tags=tags, description=description
    )
...
def mock_dependencies():
    ...
    chunker.chunk_with_structure = MagicMock()
...
def document_service(mock_dependencies, monkeypatch):
    ...
    # Mock settings - use MagicMock without spec to allow all attributes
    ...
    mock_settings.text_collection = "documents"
...
class TestUniqueDocumentIdentifiers:
    ...
    @pytest.mark.asyncio
    @given(num_documents=st.integers(min_value=2, max_value=20))
    @settings(
        max_examples=100,
        deadline=None,
    ...
    async def test_property_1_unique_document_identifiers(
        ...
        for i in range(num_documents):
            ...
            mock_dependencies["chunker"].chunk_with_structure.return_value = chunks
...
class TestUploadMetadataPersistence:
    ...
    @pytest.mark.asyncio
    @given(metadata=document_metadata_strategy())
    @settings(
        max_examples=100,
        deadline=None,
        suppress_health_check=[HealthCheck.function_scoped_fixture, HealthCheck.too_slow],
...
class TestConcurrentUploadIndependence:
    ...
    async def test_property_4_concurrent_upload_independence(
        ...
        mock_dependencies["chunker"].chunk_with_structure.side_effect = chunk_document_side_effect
        mock_dependencies["openai_client"].embed_batch.return_value = [[0.1] * 384]
        mock_dependencies["vector_store"].add_embeddings.return_value = True
...
class TestProcessingStatusAccuracy:
    ...
    async def test_property_25_processing_status_accuracy(
        ...
        if stage_to_fail == ProcessingStatus.CHUNKING:
            ...
            mock_dependencies["chunker"].chunk_with_structure.side_effect = Exception(
                "Chunking failed"
            )
        else:
            chunks = [
        ...
        if stage_to_fail == ProcessingStatus.EMBEDDING:
            mock_dependencies["openai_client"].embed_batch.side_effect = Exception(
                "Embedding failed"
            )
        else:
            mock_dependencies["openai_client"].embed_batch.return_value = [[0.1] * 384]
        ...
        if stage_to_fail == ProcessingStatus.STORING:
            mock_dependencies["vector_store"].add_embeddings.side_effect = Exception(
                "Storing failed"
            )
        else:
            mock_dependencies["vector_store"].add_embeddings.return_value = True
