...
def test_property_23_invalid_config_fails_at_startup(chunk_size: int, chunk_overlap: int):
    """Property 23: Configuration validation at startup.
...
@given(
    api_title=st.text(
        ...
        ),  # Exclude control characters including null
    ),
    api_version=st.text(
        min_size=1, max_size=20, alphabet=st.characters(blacklist_categories=("Cs", "Cc"))
    ),
