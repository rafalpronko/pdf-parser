...
def diagnose_pdf(pdf_path: Path):
    ...
    if parsed_doc.tables:
        ...
        for i, table in enumerate(parsed_doc.tables[:3], 1):
            print(f"  {i}. Page {table.page}, Rows: {len(table.rows)}, Cols: {len(table.rows[0]) if table.rows else 0}")
            if table.rows:
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print("  python scripts/diagnose_pdf.py 'data/raw/WARTA_OWU_AutoCasco_Standard_ACS_C6201_IPID_dla_umow_zawieranych_do_31.03.2022.pdf'")
        sys.exit(1)
