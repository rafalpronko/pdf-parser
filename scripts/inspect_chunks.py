...
async def main():
    ...
    for i, chunk in enumerate(page_0_chunks, 1):
        ...
        has_uszkodzenia = "uszkodzenia" in chunk.content.lower() or "uszkodzenie" in chunk.content.lower()
        has_kradzież = "kradzież" in chunk.content.lower()
        has_all_risks = "all risks" in chunk.content.lower()
        
        print(f"\n🔍 Contains:")
        if has_pojazd: print("  ✓ pojazd")
        if has_uszkodzenia: print("  ✓ uszkodzenia")
        if has_kradzież: print("  ✓ kradzież")
        if has_all_risks: print("  ✓ all risks")
